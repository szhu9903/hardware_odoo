...
def sys_param_init():
    """ 初始化系统参数 """
    thread_data.connection = PgsqlPool.get_idle_connection()
    try:
        ...
        PgsqlPool.resume_idle_connection(thread_data.connection)

# 初始化设备分类
def reflesh_equiptype():
    """ 初始化设备节点列表 """
    query_sql = "SELECT * FROM hardware_type"
    equip_types = PgsqlPool.query_data_db(query_sql)
...
def reflesh_equipid():
    ...
    data_set = PgsqlPool.query_data_db(query_sql)
